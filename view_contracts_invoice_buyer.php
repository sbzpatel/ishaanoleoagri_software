...
header('Location:index.php');
	...
	//$sql_buyer_invoice_number=("SELECT MAX(number) FROM buyer_invoice");
	...
	//echo "<pre>";
	//var_dump($seller_invoice_number);
	//echo "<pre>";
	//exit;
	//$current_invoice_number=$buyer_invoice_number + 1;
	
	


        $invoice_date=$_POST['invoice_date'];
		$invoice_company_name=$buyer_name;
		$gross_amount=0;
		$payment_status='Not-paid';



		$mysql_insert_invoice_contract="insert into buyer_invoice(`date`,`buyer_name`,`gross_amount`,`payment_status`) values('$invoice_date','$invoice_company_name','$gross_amount','$payment_status')";	
		//exit;
...
foreach($contract_id_list as $contract_id)
{
       ...
       while($row_product_query=mysql_fetch_array($product_query))
       {
             ...
             $seller_name_sql="select name from buyer where id='$contract_buyer_id';";			 
             $seller_name_query=mysql_query($seller_name_sql);
             $seller_name_row=mysql_fetch_array($seller_name_query);			 
             $sellerr_name=$seller_name_row['name'];
 ...
 $net_amount += $_POST['total'][$contract_id.$product_id];        
	 ...
	 // echo "<pre>";
	  $mysql_insert_invoice_contract_products="insert into buyer_invoice_products(`invoice_number`,`contract_number`,`product_id`,`broker_rate`,`total_amount`) values('$current_invoice_number','$contract_id','$product_id','$broker_rate','$total_amount')";	
	  //echo "</pre>";
	  //exit;
...
$update_buyer_invoice_gross_amount="update buyer_invoice set gross_amount='$gross_amount' where number='$current_invoice_number'";
$updated_buyer_invoice=mysql_query($update_buyer_invoice_gross_amount);

header("location:view_invoices_buyer.php");
