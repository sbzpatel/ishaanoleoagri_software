...
$html = " ";
	  ...
	  $sql_seller_invoice_number=("SELECT * from seller_invoice where number='$invoice_number'");
	  $seller_invoice=mysql_query($sql_seller_invoice_number);
	  $row_seller_invoice = mysql_fetch_array($seller_invoice);
	  $invoice_date=$row_seller_invoice['seller_date'];
	  $invoice_company_name=$row_seller_invoice['seller_name'];
	  
      $sql_seller_company=("SELECT * from seller where name='$invoice_company_name'");
	  ...
	  $row_seller_company = mysql_fetch_array($seller_company);
		...
		$company_email1=explode(",",$company_email);
		...
		$buyer_email=$buyer_name_row['email'];
			...
			$sql_currency=mysql_query("SELECT seller_invoice_products.contract_number,contract.currency,contract.id from seller_invoice_products inner join contract on seller_invoice_products.contract_number = contract.id"); 
...
<h5>Email : enquiry@ishaanoleoagri.com</h5>
<h5>Website : www.ishaanoleoagri.com</h5>
</td>
...
$html.='<h2 align="center"><u>Final Invoice</u></h2>';
...
<th width="10%" style="border: 0.1mm solid #888888;">
<span style="font-size:9pt; color: #000000; font-family: calibri;">Final Qty</span> 
</th>
<th width="10%" style="border: 0.1mm solid #888888;">
...
$count = 1;		
				...
				$view_contracts_sql=mysql_query("SELECT * FROM `seller_invoice_products` where invoice_number='$invoice_number'"); 
					...
					$html.='<tr>';
						...
						//exit;
						 $product_name_sql=mysql_query("SELECT * FROM `contract_product` where contract_id='$contract_number' AND product_id='$product_id'");
						  ...
						  $html.='<td style="text-align:center; border: 0.1mm solid #888888; ">'.$buyer_name.'</td>';
						  $html.='<td style="text-align:center; border: 0.1mm solid #888888; ">'.$product_rate.'</td>';
						  $html.='<td style="text-align:center; border: 0.1mm solid #888888; ">'.$product_broker_rate.'.00</td>';
						  $html.='<td style="text-align:center; border: 0.1mm solid #888888; ">'.$total_amount.'</td>';
...
$filename = $invoice_company_name."(Final).pdf";
...
for($i=0;$i<count($company_email1);$i++){

// Email settings
$mailto = $company_email1[$i];
$from_name = "Ishaan Oleo & Agri Resources";
$from_mail = 'enquiry@ishaanoleoagri.com';
$replyto = 'enquiry@ishaanoleoagri.com';
$uid = md5(uniqid(time())); 

$subject= "Invoice(Final) From Ishan Oleo & Agri Resources For ".$invoice_company_name;

$message = '<h3><b>Invoice Details (Final)</b></h3><br>';
$message.="<b>Invoice Number :</b> SC-" .$invoice_number."<br>";
$message.="<b>Invoice Date :</b> " .$invoice_date."<br>";
$message.="<b>Company Name :</b> " .$invoice_company_name."<br>";
$message.="<br><b>Detailed Information is in below Attatchment.</b>";
