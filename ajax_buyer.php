...
$html = "";
$buyer_contracts = $_GET['buyer_contracts'];
	...
	$html .='<tbody>';
			...
			$a=1;
			if($buyer_contracts == '0')
			{
				$view_contracts_sql=mysql_query("select * from contract WHERE id NOT IN (select DISTINCT contract_number from buyer_invoice_products) order by id DESC");
			}
			else	
			{
			$view_contracts_sql=mysql_query("select * from contract WHERE id NOT IN (select DISTINCT contract_number from buyer_invoice_products) and buyer_name='$buyer_contracts' order by id DESC");
			}
		...
		$html .='<tr>
				...
				$html .='<table border="1" width="100%">';
                    ...
                    <td width="13%" style="text-align:center;"><input type="text" class="form-control" name="total['.$contract_id.$product_id.']" id="total_amount'.$id.'" width="10px;" onkeyup="cal_broker_amount('.$product_quantity.','.$id.')" /></td>
					';
